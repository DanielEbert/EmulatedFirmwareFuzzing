...
avr_logger_p avr_global_logger_get(void) { return _avr_global_logger; }
...
static void _avr_io_console_write(struct avr_t *avr, avr_io_addr_t addr,
  ...
  if (avr->io_console_buffer.len + 1 >= avr->io_console_buffer.size) {
    ...
    avr->io_console_buffer.buf = (char *)realloc(avr->io_console_buffer.buf,
...
void avr_loadcode(avr_t *avr, uint8_t *code, uint32_t size,
  ...
  if ((address + size) > avr->flashend + 1) {
    AVR_LOG(avr, LOG_ERROR,
            "avr_loadcode(): Attempted to load code of size %d but flash size "
            "is only %d.\n",
            size, avr->flashend + 1);
    abort();
  }
