...
static int gdb_watch_add_or_update(avr_gdb_watchpoints_t *w,
                                   enum avr_gdb_watch_type kind, uint32_t addr,
                                   uint32_t size) {
 ...
 */
static int gdb_watch_rm(avr_gdb_watchpoints_t *w, enum avr_gdb_watch_type kind,
                        uint32_t addr) {
  int i = gdb_watch_find(w, addr);
  if (i == -1) {
    return -1;
...
static void gdb_watch_clear(avr_gdb_watchpoints_t *w) { w->len = 0; }
...
static void gdb_send_quick_status(avr_gdb_t *g, uint8_t signal) {
  ...
  sprintf(cmd, "T%02x20:%02x;21:%02x%02x;22:%02x%02x%02x00;",
          signal ? signal : 5, sreg, g->avr->data[R_SPL], g->avr->data[R_SPH],
          g->avr->pc & 0xff, (g->avr->pc >> 8) & 0xff,
          (g->avr->pc >> 16) & 0xff);
  gdb_send_reply(g, cmd);
}

static int gdb_change_breakpoint(avr_gdb_watchpoints_t *w, int set,
                                 enum avr_gdb_watch_type kind, uint32_t addr,
                                 uint32_t size) {
...
static int gdb_read_register(avr_gdb_t *g, int regi, char *rep) {
  ...
  case 33:
    ...
    break;
  case 34:
    sprintf(rep, "%02x%02x%02x00", g->avr->pc & 0xff, (g->avr->pc >> 8) & 0xff,
            (g->avr->pc >> 16) & 0xff);
    break;
  }
  return strlen(rep);
...
static void gdb_handle_command(avr_gdb_t *g, char *cmd) {
  ...
  case 'q':
    if (strncmp(cmd, "Supported", 9) == 0) {
      ...
      if (n < 32) {
        ...
        snprintf(rep, sizeof(rep),
                 "name:r%ld;bitsize:8;offset:0;encoding:uint;format:hex;set:"
                 "General Purpose Registers;gcc:%ld;dwarf:%ld;",
                 n, n, n);
        gdb_send_reply(g, rep);
        break;
      } else if (n == 32) {
        // SREG (flags) register.
        snprintf(rep, sizeof(rep),
                 "name:sreg;bitsize:8;offset:0;encoding:uint;format:binary;set:"
                 "General Purpose Registers;gcc:32;dwarf:32;");
        gdb_send_reply(g, rep);
        break;
      } else if (n == 33) {
        // SP register (SPH and SPL combined).
        snprintf(rep, sizeof(rep),
                 "name:sp;bitsize:16;offset:0;encoding:uint;format:hex;set:"
                 "General Purpose Registers;gcc:33;dwarf:33;generic:sp;");
        gdb_send_reply(g, rep);
        break;
      } else if (n == 34) {
        // PC register
        snprintf(rep, sizeof(rep),
                 "name:pc;bitsize:32;offset:0;encoding:uint;format:hex;set:"
                 "General Purpose Registers;gcc:34;dwarf:34;generic:pc;");
        gdb_send_reply(g, rep);
        break;
      } else {
  ...
  case 'M': { // write memory
    ...
    if (addr < 0xffff) {
      ...
      read_hex_string(start + 1, avr->data + addr - 0x800000,
                      strlen(start + 1));
      gdb_send_reply(g, "OK");
    } else if (addr >= 0x810000 && (addr - 0x810000) <= avr->e2end) {
      read_hex_string(start + 1, (uint8_t *)rep, strlen(start + 1));
      avr_eeprom_desc_t ee = {
          .offset = (addr - 0x810000), .size = len, .ee = (uint8_t *)rep};
  ...
  case 'z': {
    ...
    case 1: // hardware breakpoint
      if (addr > avr->flashend ||
          gdb_change_breakpoint(&g->breakpoints, set, 1 << kind, addr, len) ==
              -1) {
        gdb_send_reply(g, "E01");
        break;
      }
    ...
    case 4: // access watchpoint
      ...
      addr &= ~0x800000;
      if (addr > avr->ramend ||
          gdb_change_breakpoint(&g->watchpoints, set, 1 << kind, addr, len) ==
              -1) {
        gdb_send_reply(g, "E01");
        break;
      }
...
/**
 * If an applicable watchpoint exists for addr, stop the cpu and send a status
 * report. type is one of AVR_GDB_WATCH_READ, AVR_GDB_WATCH_WRITE depending on
 * the type of access.
 */
void avr_gdb_handle_watchpoints(avr_t *avr, uint16_t addr,
  ...
  if (kind & type) {
    ...
    READ_SREG_INTO(g->avr, sreg);
    sprintf(cmd, "T%02x20:%02x;21:%02x%02x;22:%02x%02x%02x00;%s:%06x;", 5, sreg,
            g->avr->data[R_SPL], g->avr->data[R_SPH], g->avr->pc & 0xff,
            (g->avr->pc >> 8) & 0xff, (g->avr->pc >> 16) & 0xff,
            kind & AVR_GDB_WATCH_ACCESS
                ? "awatch"
