...
int dont_trace(const char *name) {
  return (!strcmp(name, "uart_putchar") || !strcmp(name, "fputc") ||
          !strcmp(name, "printf") || !strcmp(name, "vfprintf") ||
...
void avr_core_watch_write(avr_t *avr, uint16_t addr, uint8_t v) {
  if (addr > avr->ramend) {
    AVR_LOG(avr, LOG_WARNING,
            ...
            avr->pc, _avr_sp_get(avr), _avr_flash_read16le(avr, avr->pc), v,
  ...
  if (avr->trace_data->stack_frame_index > 1 &&
      addr >
          avr->trace_data->stack_frame[avr->trace_data->stack_frame_index - 2]
              .sp) {
...
uint8_t avr_core_watch_read(avr_t *avr, uint16_t addr) {
  if (addr > avr->ramend) {
    AVR_LOG(
        ...
        avr->pc, _avr_sp_get(avr), _avr_flash_read16le(avr, avr->pc), addr,
        (avr->ramend + 1), addr % (avr->ramend + 1));
    addr = addr % (avr->ramend + 1);
  }
...
static inline void _avr_set_r16le(avr_t *avr, uint16_t r, uint16_t v) {
  _avr_set_r(avr, r, v);
  _avr_set_r(avr, r + 1, v >> 8);
}

static inline void _avr_set_r16le_hl(avr_t *avr, uint16_t r, uint16_t v) {
  _avr_set_r(avr, r + 1, v >> 8);
  _avr_set_r(avr, r, v);
}
...
const char *reg_names[255] = {
    [R_XH] = "XH",   [R_XL] = "XL",   [R_YH] = "YH",
    [R_YL] = "YL",   [R_ZH] = "ZH",   [R_ZL] = "ZL",
    [R_SPH] = "SPH", [R_SPL] = "SPL", [R_SREG] = "SREG",
};
...
#define get_vd5(o) get_d5(o) const uint8_t vd = avr->data[d];
...
#define get_vh4_k8(o) get_h4_k8(o) const uint8_t vh = avr->data[h];

#define get_d5_q6(o)                                                           \
  get_d5(o) const uint8_t q =                                                  \
      ((o & 0x2000) >> 8) | ((o & 0x0c00) >> 7) | (o & 0x7);
...
static void _avr_flags_add_zns(struct avr_t *avr, uint8_t res, uint8_t rd,
                               uint8_t rr) {
  /* carry & half carry */
...
static void _avr_flags_sub_zns(struct avr_t *avr, uint8_t res, uint8_t rd,
                               uint8_t rr) {
  /* carry & half carry */
...
static void _avr_flags_sub_Rzns(struct avr_t *avr, uint8_t res, uint8_t rd,
                                uint8_t rr) {
  /* carry & half carry */
  uint8_t sub_carry = (~rd & rr) | (rr & res) | (res & ~rd);
  avr->sreg[S_H] = (sub_carry >> 3) & 1;
 ...
 * As I went along, I noticed "bit patterns" that could be used to factor
 * opcodes However, a lot of these only became apparent later on, so SOME
 * instructions (skip of bit set etc) are compact, and some could use some
 * refactoring (the ALU ones scream to be factored). I assume that the decoder
 * could easily be 2/3 of it's current size.
 *
 * + It lacks the "extended" XMega jumps.
 * + It also doesn't check whether the core it's
 *   emulating is supposed to have the fancy instructions, like multiply and
 * such.
 *
...
run_one_again:
  ...
  if (!side_effect_sets_new_pc) {
    new_pc = avr->pc + 2; // future "default" pc
  }

  // check avr struct for what to do
  // We can calc the target addr, ret addr, ... here
  // switch (opcode & 0xf000) {
  //	case 0x9000: {
  //		/* this is an annoying special case, but at least these lines
  // handle all the SREG set/clear opcodes */ 		if ((opcode & 0xff0f) ==
  // 0x9408) { 		} else
  // switch (opcode) { 			case 0x9409:   // IJMP -- Indirect jump
  // -- 1001 0100 0000
  // 1001 			case 0x9419:   // EIJMP -- Indirect jump -- 1001
  // 0100
  // 0001 1001   bit 4 is "indirect" 			case 0x9509:   // ICALL
  // -- Indirect Call to Subroutine -- 1001 0101 0000 1001 case 0x9519: { //
  // EICALL -- Indirect Call to Subroutine -- 1001 0101 0001 1001   bit 8 is
  // "push pc"
  //				//int e = opcode & 0x10;
  //				//int p = opcode & 0x100;
  //				//if (e && !avr->eind)
  //				//	_avr_invalid_opcode(avr);
  //				//uint32_t z = avr->data[R_ZL] |
  //(avr->data[R_ZH]
  //<< 8);
  //				//if (e)
  ...
  //						// new_pc += 2;
  //						// cycle += 1 +
  //_avr_push_addr(avr, new_pc);
  //						// printf("-- Call cur_pc %d,
  // next_pc %d, ret_pc %d\n", avr->pc, a << 1, new_pc);
  //						// new_pc = a << 1;
  //						// TRACE_JUMP();
  //						// STACK_FRAME_PUSH();
  ...
  //		// cycle += _avr_push_addr(avr, new_pc);
  //		// new_pc = (new_pc + o) % (avr->flashend+1);
  //		// // 'rcall .1' is used as a cheap "push 16 bits of room on the
  // stack"
  //		// if (o != 0) {
  //		// 	TRACE_JUMP();
  //		// 	STACK_FRAME_PUSH();
      ...
      default:
        ...
        case 0x0300: { // MUL -- Multiply -- 0000 0011 fddd frrr
          ...
          case 0x00: // MULSU -- Multiply Signed Unsigned -- 0000 0011 0ddd 0rrr
            ...
            T(name = "mulsu";)
            break;
          case 0x08: // FMUL -- Fractional Multiply Unsigned -- 0000 0011 0ddd
                     // 1rrr
          ...
          case 0x80: // FMULS -- Multiply Signed -- 0000 0011 1ddd 0rrr
            ...
            T(name = "fmuls";)
            break;
          case 0x88: // FMULSU -- Multiply Signed Unsigned -- 0000 0011 1ddd
                     // 1rrr
            res = ((uint8_t)avr->data[r]) * ((int8_t)avr->data[d]);
            c = (res >> 15) & 1;
            res <<= 1;
  ...
  case 0x1000: {
    ...
    case 0x1000: { // CPSE -- Compare, skip if equal -- 0001 00rd dddd rrrr
      ...
      STATE("cpse %s[%02x], %s[%02x]\t; Will%s skip\n", avr_regname(d),
            avr->data[d], avr_regname(r), avr->data[r], res ? "" : " not");
      if (res) {
  ...
  case 0x9000: {
    ...
    if ((opcode & 0xff0f) == 0x9408) {
      ...
      case 0x9419:   // EIJMP -- Indirect jump -- 1001 0100 0001 1001   bit 4 is
                     // "indirect"
      ...
      default: {
        switch (opcode & 0xfe0f) {
        case 0x9000: { // LDS -- Load Direct from Data Space, 32 bits -- 1001
                       // 0000 0000 0000
          get_d5(opcode);
        ...
        case 0x900c:
        case 0x900d:
        case 0x900e: { // LD -- Load Indirect from Data using X -- 1001 000d
                       // dddd 11oo
          int op = opcode & 3;
          get_d5(opcode);
        ...
        case 0x920c:
        case 0x920d:
        case 0x920e: { // ST -- Store Indirect Data Space X -- 1001 001d dddd
                       // 11oo
          int op = opcode & 3;
          get_vd5(opcode);
          uint16_t x = (avr->data[R_XH] << 8) | avr->data[R_XL];
        ...
        case 0x9200: { // STS -- Store Direct to Data Space, 32 bits -- 1001
                       // 0010 0000 0000
          get_vd5(opcode);
          uint16_t x = _avr_flash_read16le(avr, new_pc);
          new_pc += 2;
        ...
        case 0x9201:
        case 0x9202: { // ST -- Store Indirect Data Space Z -- 1001 001d dddd
                       // 00oo
          int op = opcode & 3;
          get_vd5(opcode);
          uint16_t z = (avr->data[R_ZH] << 8) | avr->data[R_ZL];
        ...
        case 0x940e:
        case 0x940f: { // CALL -- Long Call to sub, 32 bits -- 1001 010a aaaa
                       // 111a
          avr_flashaddr_t a = ((opcode & 0x01f0) >> 3) | (opcode & 1);
          uint16_t x = _avr_flash_read16le(avr, new_pc);
          a = (a << 16) | x;
  ...
  if ((avr->state == cpu_Running) && (avr->run_cycle_count > cycle) &&
