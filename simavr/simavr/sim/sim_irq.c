...
typedef struct avr_irq_hook_t {
  ...
  struct avr_irq_t
      *chain; // raise the IRQ on this too - optional if "notify" is on
  avr_irq_notify_t
      notify;  // called when IRQ is raised - optional if "chain" is on
...
static void _avr_irq_pool_add(avr_irq_pool_t *pool, avr_irq_t *irq) {
  ...
  if (insert == pool->count) {
    if ((pool->count & 0xf) == 0) {
      pool->irq = (avr_irq_t **)realloc(pool->irq, (pool->count + 16) *
                                                       sizeof(avr_irq_t *));
    }
    pool->count++;
  }
...
void avr_init_irq(avr_irq_pool_t *pool, avr_irq_t *irq, uint32_t base,
...
void avr_irq_register_notify(avr_irq_t *irq, avr_irq_notify_t notify,
                             void *param) {
  if (!irq || !notify)
    return;
...
void avr_irq_unregister_notify(avr_irq_t *irq, avr_irq_notify_t notify,
                               void *param) {
...
void avr_raise_irq_float(avr_irq_t *irq, uint32_t value, int floating) {
  if (!irq)
    return;
...
uint8_t avr_irq_get_flags(avr_irq_t *irq) { return irq->flags; }

void avr_irq_set_flags(avr_irq_t *irq, uint8_t flags) { irq->flags = flags; }
