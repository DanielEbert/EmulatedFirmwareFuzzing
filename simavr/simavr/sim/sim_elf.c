...
void avr_load_firmware(avr_t *avr, elf_firmware_t *firmware) {
  ...
  if (firmware->eeprom && firmware->eesize) {
    avr_eeprom_desc_t d = {
        .ee = firmware->eeprom, .offset = 0, .size = firmware->eesize};
  ...
  avr_vcd_init(
      avr, firmware->tracename[0] ? firmware->tracename : "gtkwave_trace.vcd",
  ...
  for (int ti = 0; ti < firmware->tracecount; ti++) {
    if (firmware->trace[ti].kind == AVR_MMCU_TAG_VCD_PORTPIN) {
      avr_irq_t *irq =
          avr_io_getirq(avr, AVR_IOCTL_IOPORT_GETIRQ(firmware->trace[ti].mask),
      ...
      if (irq) {
        ...
        avr_vcd_add_signal(
            avr->vcd, irq, 1,
            firmware->trace[ti].name[0] ? firmware->trace[ti].name : name);
      }
      ...
      if (bit && firmware->trace[ti].addr < AVR_INT_IRQ_COUNT)
        avr_vcd_add_signal(avr->vcd, &bit[firmware->trace[ti].addr],
      ...
      // easy one
      avr_irq_t *all =
          avr_iomem_getirq(avr, firmware->trace[ti].addr,
                           firmware->trace[ti].name, AVR_IOMEM_IRQ_ALL);
      if (!all) {
      ...
      for (int bi = 0; bi < 8; bi++)
        if (firmware->trace[ti].mask & (1 << bi)) {
          avr_irq_t *bit = avr_iomem_getirq(avr, firmware->trace[ti].addr,
                                            firmware->trace[ti].name, bi);
          if (!bit) {
            AVR_LOG(avr, LOG_ERROR,
                    "ELF: %s: unable to attach trace to address %04x\n",
                    __FUNCTION__, firmware->trace[ti].addr);
  ...
  // if the firmware has specified a command register, do NOT start the trace
  // here the firmware probably knows best when to start/stop it
  if (!firmware->command_register_addr)
    avr_vcd_start(avr->vcd);
}

static void elf_parse_mmcu_section(elf_firmware_t *firmware, uint8_t *src,
                                   uint32_t size) {
  //	hdump(".mmcu", src, size);
  while (size) {
    uint8_t tag = *src++;
...
static int elf_copy_section(const char *name, Elf_Data *data, uint8_t **dest) {
...
int elf_read_firmware(const char *file, elf_firmware_t *firmware,
  ...
  Elf_Data *data_data = NULL, *data_text = NULL,
  ...
  // Iterate through section headers again this time well stop when we find
  // symbols
  ...
  while ((scn = elf_nextscn(elf, scn)) != NULL) {
    ...
    if (!strcmp(name, ".text"))
      ...
      // sizeof(struct avr_mcu_t), s->d_size);
      //	avr->frequency = f_cpu;
    }
#if ELF_SYMBOLS
      ...
      for (int i = 0; i < symbol_count; i++) {
        ...
        if (ELF32_ST_BIND(sym.st_info) == STB_GLOBAL ||
          ...
          if (insert == -1)
            ...
            memmove(firmware->symbol + insert + 1, firmware->symbol + insert,
                    (firmware->symbolcount - insert) *
                        sizeof(firmware->symbol[0]));
  ...
  uint32_t offset = 0;
  firmware->flashsize =
      (data_text ? data_text->d_size : 0) + (data_data ? data_data->d_size : 0);
