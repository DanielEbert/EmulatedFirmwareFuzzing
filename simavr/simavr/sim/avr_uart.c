...
static inline void avr_uart_clear_interrupt(avr_t *avr,
  ...
  if (p->udrc.vector) { // UDRE is disabled in the LIN mode
    if (p->tx_cnt) {
      ...
      if (avr_regbit_get(avr, p->txen)) {
        ...
        } else      // udrc (alias udre) should be rased repeatedly while output
                    // buffer is empty
...
static uint8_t avr_uart_status_read(struct avr_t *avr, avr_io_addr_t addr,
                                    void *param) {
  avr_uart_t *p = (avr_uart_t *)param;
  ...
  if (addr == p->rxc.raised.reg) {
    ...
    //
    uint8_t ri =
        !avr_regbit_get(avr, p->rxen) || !avr_regbit_get(avr, p->rxc.raised);
    uint8_t ti =
        !avr_regbit_get(avr, p->txen) || !avr_regbit_get(avr, p->txc.raised);

    if (p->flags & AVR_UART_FLAG_POLL_SLEEP) {

      if (ri && ti) {
        // removed for increased fuzzing speed
        //	usleep(1);
      }
    }
...
static uint8_t avr_uart_read(struct avr_t *avr, avr_io_addr_t addr,
                             void *param) {
  avr_uart_t *p = (avr_uart_t *)param;
  uint8_t v = 0;
  ...
  if (!avr_regbit_get(avr, p->rxen) ||
      !avr_regbit_get(avr,
                      p->rxc.raised) // rxc flag not raised - nothing to read!
  ) {
  ...
  if (!uart_fifo_isempty(&p->input)) { // probably redundant check
    ...
    if ((p->rx_cnt > 1) && // UART actually has 2-character rx buffer
      ...
      // prevent the firmware from reading input characters with non-realistic
      // high speed
...
static void avr_uart_baud_write(struct avr_t *avr, avr_io_addr_t addr,
  ...
  uint32_t val =
      avr_regbit_get(avr, p->ubrrl) | (avr_regbit_get(avr, p->ubrrh) << 8);

  const int databits[] = {5, 6, 7, 8, /* 'reserved', assume 8 */ 8, 8, 8, 9};
  int db = databits[avr_regbit_get(avr, p->ucsz) |
                    (avr_regbit_get(avr, p->ucsz2) << 2)];
  int sb = 1 + avr_regbit_get(avr, p->usbs);
  int word_size =
      1 /* start */ + db /* data bits */ + 1 /* parity */ + sb /* stops */;
  ...
  AVR_LOG(avr, LOG_TRACE,
          "UART: %c configured to %04x = %.4f bps (x%d), %d data %d stop\n",
...
static void avr_uart_udr_write(struct avr_t *avr, avr_io_addr_t addr, uint8_t v,
                               void *param) {
  avr_uart_t *p = (avr_uart_t *)param;
  ...
  if (p->flags & AVR_UART_FLAG_STDIO) {
    ...
    if (v == '\n' || p->stdio_len == maxsize) {
      p->stdio_len = 0;
      AVR_LOG(avr, LOG_OUTPUT, FONT_GREEN "%s\n" FONT_DEFAULT, p->stdio_out);
    }
  }
  ...
  if (avr_regbit_get(avr, p->txen)) {
    ...
    p->tx_cnt++;
    if (p->tx_cnt >
        2) // AVR actually has 1-character UART tx buffer, plus shift register
      AVR_LOG(avr, LOG_TRACE, "UART%c: tx buffer overflow %d\n", p->name,
              (int)p->tx_cnt);
    if (avr_cycle_timer_status(avr, avr_uart_txc_raise, p) == 0)
      avr_cycle_timer_register(avr, p->cycles_per_byte, avr_uart_txc_raise,
                               p); // start the tx pump
  }
}

static void avr_uart_write(struct avr_t *avr, avr_io_addr_t addr, uint8_t v,
                           void *param) {
  avr_uart_t *p = (avr_uart_t *)param;
  ...
  if (p->udrc.vector && (!udrce && new_udrce) && new_txen) {
    // If enabling the UDRC (alias is UDRE) interrupt, raise it immediately if
    // FIFO is empty. If the FIFO is not empty (clear timer is flying) we don't
  ...
  if (new_rxen != rxen) {
    if (new_rxen) {
      if (uart_fifo_isempty(&p->input)) {
        // if reception is enabled and the fifo is empty, tell whomever there is
        // room
        avr_raise_irq(p->io.irq + UART_IRQ_OUT_XOFF, 0);
        avr_raise_irq(p->io.irq + UART_IRQ_OUT_XON, 1);
      }
...
static void avr_uart_irq_input(struct avr_irq_t *irq, uint32_t value,
                               void *param) {
  avr_uart_t *p = (avr_uart_t *)param;
  avr_t *avr = p->io.avr;
  ...
  if (uart_fifo_isempty(&p->input) &&
    ...
    avr_cycle_timer_register(avr, p->cycles_per_byte, avr_uart_rxc_raise,
                             p); // start the rx pump
    p->rx_cnt = 0;
    avr_regbit_clear(avr, p->dor);
  ...
  if (!avr_regbit_get(
          avr, p->dor)) { // otherwise newly received character must be rejected
    uart_fifo_write(&p->input, value); // add to fifo
  } else {
    AVR_LOG(avr, LOG_ERROR,
            "UART%c: %s: RX buffer overrun, lost char=%c=0x%02X\n", p->name,
            __func__, (char)value, (uint8_t)value);
  }

  TRACE(printf("UART IRQ in %02x (%d/%d) %s\n", value, p->input.read,
               p->input.write, uart_fifo_isfull(&p->input) ? "FULL!!" : "");)
