  ...
  if (vector->pending) {
    if (vector->trace)
      printf("IRQ%d:I=%d already raised (enabled %d) (cycle %lld pc 0x%x)\n",
             vector->vector, !!avr->sreg[S_I],
             avr_regbit_get(avr, vector->enable), (long long int)avr->cycle,
             avr->pc);
...
void avr_clear_interrupt(avr_t *avr, avr_int_vector_t *vector) {
  ...
  avr_raise_irq_float(
      avr->interrupts.irq + AVR_INT_IRQ_PENDING,
      avr_has_pending_interrupts(avr)
          ? avr_int_pending_read_at(&avr->interrupts.pending, 0)->vector
          : 0,
      !avr_has_pending_interrupts(avr));
...
void avr_interrupt_reti(struct avr_t *avr) {
  ...
  avr_raise_irq(table->irq + AVR_INT_IRQ_RUNNING,
                table->running_ptr > 0
                    ? table->running[table->running_ptr - 1]->vector
                    : 0);
  avr_raise_irq(avr->interrupts.irq + AVR_INT_IRQ_PENDING,
                avr_has_pending_interrupts(avr));
}
...
/*
 * check whether interrupts are pending. If so, check if the interrupt "latency"
 * is reached, and if so triggers the handlers and jump to the vector.
 */
void avr_service_interrupts(avr_t *avr) {
  ...
  // the one we service
  table->pending
      .buffer[(table->pending.read + mini) % avr_int_pending_fifo_size] =
