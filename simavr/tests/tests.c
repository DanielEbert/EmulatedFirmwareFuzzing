...
avr_t *tests_init_avr(const char *elfname) {
  ...
  elf_firmware_t fw;
  if (elf_read_firmware(elfname, &fw, symbols))
...
void tests_assert_uart_receive_avr(avr_t *avr, unsigned long run_usec,
                                   const char *expected, char uart) {
  struct output_buffer buf;
  init_output_buffer(&buf);

  avr_irq_register_notify(
      avr_io_getirq(avr, AVR_IOCTL_UART_GETIRQ(uart), UART_IRQ_OUTPUT),
      buf_output_cb, &buf);
  enum tests_finish_reason reason = tests_run_test(avr, run_usec);
  if (reason == LJR_CYCLE_TIMER) {
    if (strcmp(buf.str, expected) == 0) {
      _fail(NULL, 0,
            "Simulation did not finish within %lu simulated usec. "
            "UART output is correct and complete.",
            run_usec);
    }
    _fail(NULL, 0,
          "Simulation did not finish within %lu simulated usec. "
          "UART output so far: \"%s\"",
          run_usec, buf.str);
  }
  if (strcmp(buf.str, expected) != 0)
    _fail(NULL, 0, "UART outputs differ: expected \"%s\", got \"%s\"", expected,
          buf.str);
}

void tests_assert_uart_receive(const char *elfname, unsigned long run_usec,
                               const char *expected, char uart) {
  avr_t *avr = tests_init_avr(elfname);
...
void tests_assert_cycles_at_least(unsigned long n) {
  if (tests_cycle_count < n)
    _fail(NULL, 0,
          "Program ran for too few cycles (%" PRI_avr_cycle_count " < %lu)",
          tests_cycle_count, n);
}
...
void tests_assert_cycles_at_most(unsigned long n) {
  if (tests_cycle_count > n)
    _fail(NULL, 0,
          "Program ran for too many cycles (%" PRI_avr_cycle_count " > %lu)",
          tests_cycle_count, n);
}
