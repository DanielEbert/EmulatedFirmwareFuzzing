...
struct avr_t;
typedef uint8_t (*avr_io_read_t)(struct avr_t *avr, avr_io_addr_t addr,
                                 void *param);
typedef void (*avr_io_write_t)(struct avr_t *avr, avr_io_addr_t addr, uint8_t v,
                               void *param);
...
/*
 * Main AVR instance. Some of these fields are set by the AVR "Core" definition
 * files the rest is runtime data (as little as possible)
 */
typedef struct avr_t {
  const char *mmcu; // name of the AVR
  // these are filled by sim_core_declare from constants in
  ...
  // these next two allow the core to freely run between cycle timers and also
  // allows for a maximum run cycle limit... run_cycle_count is set during cycle
  // timer processing.
  ...
  struct {
    struct avr_irq_t
        *irq; // optional, used only if asked for with avr_iomem_getirq()
    struct {
      void *param;
      avr_io_read_t c;
    } r;
...
// initializes a new AVR instance. Will call the IO registers init(), and then
// reset()
...
void avr_loadcode(avr_t *avr, uint8_t *code, uint32_t size,
 ...
 */
typedef void (*avr_logger_p)(struct avr_t *avr, const int level,
...
uint64_t avr_get_time_stamp(avr_t *avr);
